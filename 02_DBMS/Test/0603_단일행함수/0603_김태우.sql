...
SELECT ENAME, HIREDATE, NEXT_DAY(ADD_MONTHS(HIREDATE, 6), '월') "6M+ MON"
    FROM EMP;

--4. 이름, 입사일, 입사일로부터 현재까지의 개월수, 급여, 입사일부터 현재까지의 받은 급여의 총계를 출력
SELECT ENAME, HIREDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) "ON DUTY",
    SAL, TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE))*SAL "TTL SAL"
    FROM EMP;
...
SELECT ENAME, JOB, HIREDATE, TO_CHAR(HIREDATE, 'DAY') "HIRE DAY"
    FROM EMP;

--7. 이름의 길이가 6자 이상인 사원의 정보를 이름,이름의 글자수,업무를 출력
SELECT ENAME, LENGTH(ENAME) "N LENGTH", JOB
    FROM EMP
    WHERE LENGTH(ENAME) >= 6;

--8. 모든 사원의 정보를 이름, 업무, 급여, 보너스, 급여+보너스를 출력
SELECT ENAME, JOB, SAL, COMM, SAL+NVL(COMM,0) "TTL SAL"
    FROM EMP;

-- 9.사원 테이블의 사원명에서 2번째 문자부터 3개의 문자를 추출하시오. 
SELECT SUBSTR(ENAME, 2, 3) "NAME 2,3"
    FROM EMP;
...
SELECT EMPNO, ENAME, JOB, TO_CHAR(SAL, '$9,999,999') SAL
    FROM EMP
    WHERE DEPTNO = 20;
