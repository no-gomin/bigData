...
head(iris$Sepal.Length, 10, drop=FALSE) # drop=FALSE가 안됨.(???)
iris[1:10, 'Sepal.Length', drop=FALSE] 
...
X <- matrix(c(12000, 26000, 18000), ncol = 3) 
X
dimnames(X) <- list(c('시급'), c('A', 'B', 'C'))

Y <- matrix(c(5, 4, 9, 7, 3, 2), nrow = 3) # A B C 시간
dimnames(Y) <- list(c('A시간', 'B시간', 'C시간'), c('갑', '을'))
Y
