# 미미국 동북중부 437개 지역의 인구통계 정보를 담고 있는 midwest 데이터를 사용해 데이터 분석 문제를 해결해 보세요. midwest는 ggplot2 패키지에 들어 있습니다.
...
midwest %>% 
  mutate(grade = ifelse(popchild >= 40, 'large', ifelse(popchild >= 30, 'middle', 'small'))) %>% 
  group_by(grade) %>% 
  summarise(n())
...
midwest %>% 
  select(poptotal, popasian, asian_per) %>% 
  arrange(asian_per) %>% 
  head(10)
